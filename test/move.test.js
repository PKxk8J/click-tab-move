...
test('設定値を正規化する', () => {
  ...
  assert.deepEqual(normalizeDestination(undefined), { type: 'newWindow' })
  assert.deepEqual(normalizeDestination({
    type: 'window',
    windowId: '2',
  }), { type: 'window', windowId: 2 })
  assert.deepEqual(normalizeDestination({
    type: 'group',
    groupId: '30',
  }), { type: 'group', groupId: 30 })
  assert.throws(() => normalizeDestination(2), /Unsupported destination/)
})
...
test('グループではグループ内のタブと分割ビューを単位にして移動対象タブを列挙する', async () => {
  ...
  assert.deepEqual(await listTargetTabIds(3, 'all', 'group'), [2, 3, 4, 5])
})
...
test('グループ内の全てのタブをウィンドウへ移す場合はグループを解体する', async () => {
  resetTabs([
    { id: 1, windowId: 1, index: 0, groupId: 10, active: true },
    { id: 2, windowId: 1, index: 1, groupId: 10 },
    { id: 20, windowId: 2, index: 0, active: true },
  ...
  await run(1, 'all', { type: 'window', windowId: 2 }, false, false, 'group')
...
test('グループ内の全てのタブを同じウィンドウへ移す場合は並びを保ってグループから出す', async () => {
  resetTabs([
    { id: 1, windowId: 1, index: 0 },
    { id: 2, windowId: 1, index: 1, groupId: 10 },
  ...
  await run(3, 'all', { type: 'window', windowId: 1 }, false, false, 'group')

  assert.deepEqual(state.moved, [])
  assert.deepEqual(state.ungrouped, [2, 3, 4])
  assert.deepEqual(state.activated, [])
  assert.deepEqual(getTabIds(1), [1, 2, 3, 4, 5])
  assert.deepEqual(getMemberships().filter((tab) => [2, 3, 4].includes(tab.id)), [
    { id: 2, windowId: 1, groupId: -1, splitViewId: -1 },
    { id: 3, windowId: 1, groupId: -1, splitViewId: -1 },
    { id: 4, windowId: 1, groupId: -1, splitViewId: -1 },
  ])
})

test('グループ内の端のタブを同じウィンドウへ移す場合は並びを保つ', async () => {
  resetTabs([
    { id: 1, windowId: 1, index: 0 },
    { id: 2, windowId: 1, index: 1, groupId: 10 },
    { id: 3, windowId: 1, index: 2, groupId: 10 },
  ...
  await run(2, 'one', { type: 'window', windowId: 1 }, false, false, 'group')

  assert.deepEqual(getTabIds(1), [1, 2, 3, 4, 5])
  assert.deepEqual(getMemberships().filter((tab) => [2, 3, 4].includes(tab.id)), [
    { id: 2, windowId: 1, groupId: -1, splitViewId: -1 },
    { id: 3, windowId: 1, groupId: 10, splitViewId: -1 },
    { id: 4, windowId: 1, groupId: 10, splitViewId: -1 },
  ])
  ...
  await run(4, 'one', { type: 'window', windowId: 1 }, false, false, 'group')

  assert.deepEqual(getTabIds(1), [1, 2, 3, 4, 5])
  assert.deepEqual(getMemberships().filter((tab) => [2, 3, 4].includes(tab.id)), [
    { id: 2, windowId: 1, groupId: 10, splitViewId: -1 },
    { id: 3, windowId: 1, groupId: 10, splitViewId: -1 },
    { id: 4, windowId: 1, groupId: -1, splitViewId: -1 },
  ])
})

test('グループ内の途中のタブを同じウィンドウへ移す場合はグループ右隣へ出す', async () => {
  resetTabs([
    { id: 1, windowId: 1, index: 0 },
    { id: 2, windowId: 1, index: 1, groupId: 10 },
    { id: 3, windowId: 1, index: 2, groupId: 10, active: true },
  ...
  await run(3, 'one', { type: 'window', windowId: 1 }, false, false, 'group')

  assert.deepEqual(state.activated, [])
  assert.deepEqual(getTabIds(1), [1, 2, 4, 5, 3, 6])
  assert.deepEqual(getMemberships().filter((tab) => [2, 3, 4, 5].includes(tab.id)), [
    { id: 2, windowId: 1, groupId: 10, splitViewId: -1 },
    { id: 3, windowId: 1, groupId: -1, splitViewId: -1 },
    { id: 4, windowId: 1, groupId: 10, splitViewId: -1 },
    { id: 5, windowId: 1, groupId: 10, splitViewId: -1 },
  ])
})
...
test('移動後フォーカスが有効な場合は移動先ウィンドウと最後の移動タブを有効化する', async () => {
  ...
  await rawRun([1], { type: 'window', windowId: 2 }, false, true)
...
test('通知権限がない場合は通知せずに移動する', async () => {
  ...
  await rawRun([1], { type: 'window', windowId: 2 }, true, false)
...
test('未対応のキーではタブを移動しない', async () => {
  ...
  const errorMock = mock.method(globalThis.console, 'error', () => {})
  try {
    await run(1, 'unknown', { type: 'window', windowId: 2 }, false, false)
  } finally {
    errorMock.mock.restore()
  }
