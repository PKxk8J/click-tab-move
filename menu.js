...

{
  ...
  async function reset () {
    ...
    switch (menuKeys.length) {
      ...
      case 1: {
        ...
        if (key !== KEY_ALL) {
          addMenuItem(key + SEP + KEY_NEW_WINDOW,
            i18n.getMessage(KEY_NEW_WINDOW), key)
        }
        break
      }
      default: {
  ...
  (async function () {
    ...
    windows.onFocusChanged.addListener(
      (windowId) => filterWindow(windowId).catch(onError))
    ...
    contextMenus.onClicked.addListener((info, tab) => (async function () {
      ...
      const toWindowId = (toWindowLabel === KEY_NEW_WINDOW ? undefined : Number(
        toWindowLabel))
