...

{
  ...
  async function moveTarget (
    tabIds, toWindowId, index, pinnedTabIds, unpinnedTabIds, focus) {
  ...
  async function runWithNewWindow (
    pinnedTabIds, unpinnedTabIds, progress, focus) {
    ...
    await moveTarget(target, windowInfo.id, index, pinnedTabIds, unpinnedTabIds,
      focus)
    ...
    (async function () {
      ...
      switch (message.type) {
        case KEY_SELECT_SIZE: {
          ...
          const selectSave = await getValue(KEY_SELECT_SAVE,
            DEFAULT_SELECT_SAVE)
          if (!selectSave) {
            break
          }
